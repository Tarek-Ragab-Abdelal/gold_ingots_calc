...
class GoldCalculatorApp {
    ...
    /**
     * Fetch gold prices from API with fallback support
     */
    async fetchPrices() {
        try {
            this.uiManager.showLoading(true);
            this.goldPrices = await this.apiService.fetchPrices();
            // Update UI with new prices
            this.uiManager.updateMarketCards(this.goldPrices);
            this.uiManager.updateProductSelect(this.goldPrices);
            // Update last updated timestamp
            const lastUpdated = this.apiService.getLastUpdated();
            if (lastUpdated) {
                this.uiManager.updateLastUpdated(lastUpdated.date, lastUpdated.time);
            }
...
parseProductWeight(product);
{
    ...
    if (name.includes('pound'))
        return 8; // Egyptian gold pound ~ 8 g
